...
def main() -> int:
        ...
        for repetition in range(args.repetitions):
            ...
            while True:
                ...
                attempt_row = {
                    ...
                    "phase": best_phase.get("phase"),
                    "frame_start": best_phase.get("frame_start"),
                    "pilot_start": best_phase.get("pilot_start"),
                    "sync_source": best_phase.get("sync_source"),
                    "payload_cycle_offset": best_phase.get("payload_cycle_offset"),
                    "frame_lock_pilots_checked": best_phase.get("frame_lock_pilots_checked"),
    ...
    summary_fields = [
        ...
        "pilot_start",
        "sync_source",
        "payload_cycle_offset",
        "frame_lock_pilots_checked",
        "validation_ber_mean",
    ...
    with csv_path.open("w", newline="", encoding="utf-8") as fh:
        ...
        for row in all_results:
            flat = {
                ...
                "phase": ",".join(str(run.get("phase")) for run in row["runs"]),
                "frame_start": ",".join(str(run.get("frame_start")) for run in row["runs"]),
                "pilot_start": ",".join(str(run.get("pilot_start")) for run in row["runs"]),
                "sync_source": ",".join(str(run.get("sync_source")) for run in row["runs"]),
                "payload_cycle_offset": ",".join(str(run.get("payload_cycle_offset")) for run in row["runs"]),
                "frame_lock_pilots_checked": ",".join(str(run.get("frame_lock_pilots_checked")) for run in row["runs"]),
